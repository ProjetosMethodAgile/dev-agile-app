...
class Parametros_Controller extends Controller {
  ...
  async pegaTodosParametros_controller(req, res) {
    try {
      const listaParametros =
        await parametros_services.pegaTodosParametros_Services();
  ...
  async pegaParametrosEmpresa_controller(req, res) {
    ...
    try {
      const listaParametros =
        await parametros_services.pegaParametrosEmpresa_Services(id);
  ...
  async criaParametros_controller(req, res) {
    ...
    try {
      if (isTrue.status) {
        ...
        const empresaValida = await empresa_services.pegaEmpresaPorId_Services(
          empresa_id
        );
        const tipoParametro =
          await permissao_services.pegaPermissaoPorId_Services(tipo_id);
        if (!empresaValida || !tipoParametro) {
          return res
            .status(400)
            .json({
              message:
                "Erro ao criar parametro, contate o administrador do sistem",
              error: true,
            });
        }
        const cadastraParametros =
          await parametros_services.criaParametros_Services(
            nome,
            empresa_id,
            descricao,
            tipo_id
        ...
        if (cadastraParametros.error) {
          return res
            .status(400)
            .json({
              message: cadastraParametros.message
                ? cadastraParametros.message
                : "Erro ao criar parametro, contate o administrador do sistem",
              error: true,
            });
        }
        return res
          .status(200)
          .json({
            parametro: cadastraParametros.parametro,
  ...
  async deletaParametrosporID_controller(req, res) {
    const { id } = req.body;
    try {
      const parametroDeletado =
        await parametros_services.deletaParametro_Services(id);
  ...
  async atualizaParametroPorNome(req, res) {
    ...
    try {
      ...
      if (!parametrosAtualizado) {
        return res.status(404).json({ message: `Parametro não encontrada` });
      }
